...
<div class="container">
	...
	<table class="table">
		...
		<c:forEach var="board" items="${boards }">
				<c:if test="${board.no eq board.pno }">
					<tr>
						<td>${board.no }</td>
					...
					<c:forEach var="childBoard" items="${boards }">
						<c:if test="${childBoard.pno eq board.no and childBoard.pno ne childBoard.no}">
							<tr>
								<td>${childBoard.no }</td>
								<td style="text-indent: 15px"><a href="detail.do?no=${childBoard.no }"> &rdsh; ${childBoard.title }</a></td>
								<td>${childBoard.writer }</td>
								<td><fmt:formatDate value="${childBoard.createdate }" pattern="yyyy.MM.dd HH:mm:ss"/></td>
							</tr>
						</c:if>
					</c:forEach>
				</c:if>
